...
<body>
    ...
    <?php
        try{
            $sql = "SELECT * FROM Product where Product in('s10')";
            ...
            while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
                ...
                echo "<th><a href='menu.php?name=s10'><img src=".$row ["Screen"]." class='gazou'></a></th>\n";
                #"</form>\n";
        ...
        <?php
        try{
            $sql = "SELECT * FROM Product where Product in('s11')";
	        $stmt = $pdo ->prepare($sql);
	        $stmt->execute();
            ...
            while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
                ...
                echo "<tr>\n";
                echo "<th><a href='menu.php?name=s11'><img src=".$row ["Screen"]." class='gazou'></a></th>\n";
                echo "<tr align='center'>\n";
                echo "<td><font size='5'>".$row["Productname"]."</td>\n";
    ...
    <?php
        try{
            $sql = "SELECT * FROM Product where Product in('s12')";
	        $stmt = $pdo ->prepare($sql);
	        $stmt->execute();

            while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
                echo "<tr>\n";
                echo "<th><a href='menu.php?name=s12'><img src=".$row ["Screen"]." class='gazou'></a></th>\n";
